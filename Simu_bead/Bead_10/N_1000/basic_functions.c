...
void calculate_autocorrelation(double** Rbb, double *stock_correl, int T_correlation, int N) {
    ...
    for (int i = 0; i < T_correlation; i++) {
        ...
        for (int j = 0; j < T_correlation - i; j++) {
            double *adjusted_vec1 = (double *)malloc(3 * sizeof(double));
            double *adjusted_vec2 = (double *)malloc(3 * sizeof(double));
            for (int k = 0; k < 3; k++) {
...
void calculate_msd(double ****R_monomere_arrays,double ***stock_msd, int Tp, int Nm, int *list_monomere, int nbr_simu, int nbr_total_simu){
        ...
        if(mkdir(folder_name,0777) == -1){
            ...
            printf("Le dossier '%s' à été crée avec succès", folder_name);
        }
        ...
        for (int i = 0; i < Tp; i++){
            ...
            for ( int k = 0; k < Nm; k++){
                for (int j = 0; j < Tp - i; j++){
                    msd += distance2(R_monomere_arrays[nbr_simu][i+j][k], R_monomere_arrays[nbr_simu][j][k]);
                }
                msd = msd/((Tp - i));

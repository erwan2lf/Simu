...
void init_sim_vars(SimVars *sv, const Config *cfg) {
    ...
    printf("adresse de R_rnap = %p \n ", sv->R_rnap);
    printf("adresse de R_rnap_new = %p \n ", sv->R_rnap_new );
...
void ajouter_rnap(SimVars *sv, const Config *cfg, NeighborList *neighbor_list, NeighborList_rnap ***p_neighbor_lists_rnap, int t){
    ...
    if(sv->nb_rnap == 0){
        ...
        creation_grosse_bille(sv->R,sv->nb_rnap, sv->positions_bille_rnap, sv->R_rnap, cfg->N, cfg->debut_segment, cfg->fin_segment, cfg->nb_rnap_initial);
    ...
    if(cfg->nb_rnap_initial > 0) {
        ...
        *p_neighbor_lists_rnap = neighbor_lists_rnap;
        build_neighbor_list_rnap_chrom(sv->R_rnap, sv->nb_rnap, cfg->rnap_subunits, sv->R, cfg->N, neighbor_lists_rnap, cfg->rayon_ecrantage_LJ_chrom, t);}
}
                    ...
                    int rnap, int prout, int t)
{
    ...
    *p_lists = lists;
    ...
    for (int sub = 0; sub < rows; ++sub) {
        // printf("sv->sv->R_rnap[old_nb-1][sub] = %lf %lf %lf \n ",sv->R_rnap[old_nb-1][sub][0], sv->R_rnap[old_nb-1][sub][1], sv->R_rnap[old_nb-1][sub][2]);
    ...
    if (new_nb > 0) {
      build_neighbor_list_rnap_chrom(
         sv->R_rnap, new_nb, cfg->rnap_subunits, sv->R,
...
// static inline void free_rnap_row(double **row, int cols) {
//     if (!row) return;
//     for (int j = 0; j < cols; ++j) free(row[j]);   // chaque pointeur vers (x,y,z)
//     free(row);                                     // tableau des pointeurs de subunits
// }
...
//     // Marqueur éventuel (si utile chez toi)
//     sv->sortie = 1;

//     // 2) Prépare tailles
//     const int old_nb = sv->nb_rnap;
//     if (old_nb <= 0) return;
//     const int new_nb = old_nb - 1;
//     const int cols   = cfg->rnap_subunits;

//     // 3) Voisinages RNAP (NeighborList_rnap)
//     NeighborList_rnap **lists = *p_lists;
//     // 3a) libère la sous-liste de l’indice retiré
//     for (int j = 0; j < cols; ++j) {
//         free(lists[rnap][j].neighbors);
//         lists[rnap][j].neighbors = NULL;
//     }
//     free(lists[rnap]);
//     lists[rnap] = NULL;
...
//     // 3c) shrink
//     if (new_nb > 0) {
//         void *tmp = realloc(lists, new_nb * sizeof *lists);
//         if (!tmp) { perror("realloc neighbor_lists_rnap"); exit(EXIT_FAILURE); }
//         lists = tmp;
//     } else {
//         free(lists);
//         lists = NULL;
//     }
    // *p_lists = lists;
    // // 4) Tableaux parallèles alignés aux indices RNAP
    // //    → il faut AUSSI décaler avant de raccourcir
    // for (int i = rnap; i < old_nb - 1; ++i) {
    //     sv->positions_bille_rnap[i]    = sv->positions_bille_rnap[i + 1];
    //     sv->avancement_transcription[i] = sv->avancement_transcription[i + 1];
    //     // si tu as un tableau sv->num_rnap aligné aux indices RNAP, décale-le aussi :
    //     // sv->num_rnap[i] = sv->num_rnap[i + 1];
    // }

    // if (new_nb > 0) {
    //     void *tmp;

    //     tmp = realloc(sv->positions_bille_rnap, new_nb * sizeof *sv->positions_bille_rnap);
    //     if (!tmp) { perror("realloc positions_bille_rnap"); exit(EXIT_FAILURE); }
    //     sv->positions_bille_rnap = tmp;
        

    //     tmp = realloc(sv->avancement_transcription, new_nb * sizeof *sv->avancement_transcription);
    //     if (!tmp) { perror("realloc avancement_transcription"); exit(EXIT_FAILURE); }
    //     sv->avancement_transcription = tmp;
    //     // Si tu gères sv->num_rnap de la même manière :
    //     // tmp = realloc(sv->num_rnap, new_nb * sizeof *sv->num_rnap);
    //     // if (!tmp) { perror("realloc num_rnap"); exit(EXIT_FAILURE); }
    //     // sv->num_rnap = tmp;
        
    // } else {
    //     free(sv->positions_bille_rnap);     sv->positions_bille_rnap     = NULL;
    //     free(sv->avancement_transcription); sv->avancement_transcription = NULL;
    //     free(sv->num_rnap); sv->num_rnap = NULL;
    // }
        // 5) R_rnap et R_rnap_new (matrices 3D)
//     // 5a) libère la ligne retirée (PAS la dernière)
//     free_rnap_row(sv->R_rnap[rnap], cols);
//     // free_rnap_row(sv->R_rnap_new[rnap], cols);

//     sv->R_rnap[rnap]     = NULL;
//     sv->R_rnap_new[rnap] = NULL;
//     // 5b) décale les pointeurs de ligne
//     for (int i = rnap; i < old_nb - 1; ++i) {
//         sv->R_rnap[i]     = sv->R_rnap[i + 1];
//         sv->R_rnap_new[i] = sv->R_rnap_new[i + 1];
//     }
//     // 5c) shrink l’enveloppe externe
//     if (new_nb > 0) {
//         void *tmp;

//         tmp = realloc(sv->R_rnap, new_nb * sizeof *sv->R_rnap);
//         if (!tmp) { perror("realloc R_rnap"); exit(EXIT_FAILURE); }
//         sv->R_rnap = tmp;
//         // tmp = realloc(sv->R_rnap_new, new_nb * sizeof *sv->R_rnap_new);
//         // if (!tmp) { perror("realloc R_rnap_new"); exit(EXIT_FAILURE); }
//         // sv->R_rnap_new = tmp;
...
//     // 6) Met à jour le nombre de RNAP
//     sv->nb_rnap = new_nb;

//     // 7) Reconstruit les neighbor-lists
//     build_neighbor_list(sv->R, neighborlist, cfg->N, 2, 0);
    
//     if (new_nb > 0) {
//         printf("new_nb = %d \n", new_nb);
//         printf("sv->R_rnap[0][0][0] = %lf ", sv->R_rnap[0][0][0]);
//         build_neighbor_list_rnap_chrom(
//             sv->R_rnap, new_nb, cfg->rnap_subunits, sv->R,
//             cfg->N, *p_lists, cfg->rayon_ecrantage_LJ_chrom, t
//         );
//         if( t == 999){printf(" \n PROUT 1\n");}
//     }
// }
...
void calcul(SimVars *sv, const Config *cfg, const Files *f, NeighborList *neighbor_lists, NeighborList_rnap **neighbor_lists_rnap, int nbr_simu, double endtoend, int nb_rnap_2, int nb_rnap_initial_2, int rnap_subunits_2){
    ...
    for (int t = 0; t < cfg->T; t++){
        ...
        if( cfg->nb_rnap_initial > 0){
            // printf("\n \n \n nb _rnap = %d et premier_passage = %d \n \n \n ", sv->nb_rnap, sv->premier_passage);
        ...
        if(cfg->nb_rnap_initial > 0){
            for(int rnap = 0; rnap < sv->nb_rnap; rnap++){
                ...
                int prout = 0;
                // polymere_brownian_motion_ring_force(sv->R_rnap[rnap], cfg->alpha, 1 * cfg->K_rnap, cfg->Delta, cfg->rnap_subunits, rnap, sv->R_rnap_new[rnap], f->test, t, cfg->periode_force, f->fichier_force_rnap, f->fichier_force_thermique);
                ...
                thermic_noise_rnap(sv, cfg, rnap);
                sv->R_rnap[rnap] = bond_rnap_bead_progressive_mvt(sv->R, sv->R_rnap[rnap], cfg->rnap_subunits, cfg->a_transpt, cfg->K_transpt, cfg->Delta, sv->R_rnap[rnap], sv->positions_bille_rnap[rnap], sv->avancement_transcription[rnap], cfg->a, cfg->alpha, f->fichier_force_lea, cfg->periode_force, t);
        ...
        // if(sv->R_rnap == sv->R_rnap_new){
        //     printf(" \n R_rnap et R_rnap_new ont la meme adresse \n");
        // }
        // else {
        //     printf(" \n R_rnap et R_rnap_new n'ont PAS la meme adresse \n");
        // }
    }
...
void f_equilibriate(SimVars *sv, const Config *cfg, const Files *f, NeighborList *neighbor_lists, NeighborList_rnap **neighbor_lists_rnap, int nbr_simu){
    ...
    for (int t = 0; t < cfg->T_eq; t++){
        ...
        if (t%1000==0){
            ...
            if(cfg->nb_rnap_initial > 0) {build_neighbor_list_rnap_chrom(sv->R_rnap, sv->nb_rnap, cfg->rnap_subunits, sv->R, cfg->N, neighbor_lists_rnap, cfg->rayon_ecrantage_LJ_chrom, t);}
...
void thermic_noise_rnap(SimVars *sv, const Config *cfg, int rnap){
    ...
    for (int i = 0; i < cfg->rnap_subunits; i++){
        for (int j = 0; j < 3; j++){
            sv->R_rnap[rnap][i][j] += sqrt(2 * cfg->Delta) * randn();
        }
    }

...
void enregistrement_RNAP(FILE* fichier, double** R, int N, double*** R_rnap, int nb_rnap, int t,
    ...
    int total_atoms = N + nb_rnap_initial * (nb_subunits + 2);  // 8 subunits + 2 monomères par RNAP
    int count = N;
    int type = 3;
...
void polymere_brownian_motion_ring_force(double** R_rnap, double alpha, double K_rnap, double Delta, int N, int rnap, double** r_new, FILE* test, int t,  int periode_enregistrement_force, FILE* fichier_force_rnap, FILE* fichier_force_thermique) {
    ...
    p = 0; pmax = 1; pmin = N-1;
    mvt_brownian_harmonic_force_RNAP(p, pmin, pmax, rnap, r_new, R_rnap, alpha, K_rnap, Delta, test, t, periode_enregistrement_force, fichier_force_rnap, fichier_force_thermique);
...
double** bond_rnap_bead_progressive_mvt(double** R, double** R_rnap, int rnap_subunits, double a_transpt, double K_transpt, double Delta, double** r_new, int mono_transcrpt, double dx_avancement_rnap, double a, double alpha, FILE* fichier_force_lea, int periode_enregistrement_force, int t) {
    double dx_suivant = 1 - dx_avancement_rnap; // déplacement vu de la particule p+1
    double a_nucl_transcrit = sqrt(a_transpt*a_transpt + dx_avancement_rnap*dx_avancement_rnap);
    double a_nucl_suivant = sqrt(a_transpt*a_transpt + dx_suivant*dx_suivant);
    double deplacement = 0.0;


    if( t % periode_enregistrement_force == 0){
        fprintf(fichier_force_lea, "RNAP : %d \n", mono_transcrpt);
    }
    
   
    for (int subunit=0; subunit<rnap_subunits; subunit+=1){
        
        double dist = distance(R[mono_transcrpt], R_rnap[subunit]);
        if (dist == 0) {dist = 1;}
        double dist_suivant = distance(R[mono_transcrpt+1], R_rnap[subunit]);

        if(t % periode_enregistrement_force == 0){
            fprintf(fichier_force_lea, "subunit : %d ", subunit);
        }
        for(int j = 0; j < 3; j++){
            if(t % periode_enregistrement_force == 0){fprintf(fichier_force_lea,"%lf ", K_transpt * (R[mono_transcrpt][j] - R_rnap[subunit][j]) * (1  - (alpha+1)/(2*dist)));}
            deplacement = - r_new[subunit][j];


            
            r_new[subunit][j] += Delta * K_transpt * (R[mono_transcrpt][j] - R_rnap[subunit][j]) * (1  - (alpha+1)/(2*dist));

            deplacement += r_new[subunit][j];
            if(t % periode_enregistrement_force == 0){fprintf(fichier_force_lea,"%lf ", deplacement);}
        }


        if(t % periode_enregistrement_force == 0){
            fprintf(fichier_force_lea, "\nMono transc : %d ", mono_transcrpt);
        }
        for(int j = 0; j < 3; j++ ){
            if(t % periode_enregistrement_force == 0){fprintf(fichier_force_lea,"%lf ", - K_transpt * (R[mono_transcrpt][j] - R_rnap[subunit][j]) * (1  - (alpha+1)/(2*dist)));}
            deplacement = - R[mono_transcrpt][j];



            R[mono_transcrpt][j] += - Delta * K_transpt * (R[mono_transcrpt][j] - R_rnap[subunit][j]) * (1  - (alpha+1)/(2*dist));
        ...
        for(int j = 0; j < 3; j++){
            if(t % periode_enregistrement_force == 0){fprintf(fichier_force_lea,"%lf ", K_transpt * (R[mono_transcrpt+1][j] - R_rnap[subunit][j]) * (1 - (alpha+1)/(2*dist_suivant)));}
            deplacement = - r_new[subunit][j];



            r_new[subunit][j] += Delta * K_transpt * (R[mono_transcrpt+1][j] - R_rnap[subunit][j]) * (1 - (alpha+1)/(2*dist_suivant));
        ...
        if(t % periode_enregistrement_force == 0){
            fprintf(fichier_force_lea, "\nMono transc+1 : %d ", mono_transcrpt + 1);
        }

        for (int j = 0; j < 3; j++) {
            if(t % periode_enregistrement_force == 0){fprintf(fichier_force_lea,"%lf ", - K_transpt * (R[mono_transcrpt+1][j] - R_rnap[subunit][j]) * (1 - (alpha+1)/(2*dist_suivant)));}
            deplacement = - R[mono_transcrpt+1][j];
            R[mono_transcrpt+1][j] += - Delta * K_transpt * (R[mono_transcrpt+1][j] - R_rnap[subunit][j]) * (1 - (alpha+1)/(2*dist_suivant));
            deplacement += R[mono_transcrpt+1][j];
            if(t % periode_enregistrement_force == 0){fprintf(fichier_force_lea,"%lf ", deplacement );}
                }
        if(t % periode_enregistrement_force == 0){
...
void lennard_jones_forces_rnap(double ***R_rnap, int nb_rnap, int rnap_subunits, double **R, int N, NeighborList_rnap **neighbor_lists, double epsilon, double sigma6, double sigma12, double sigma6rnap2, double sigma12rnap2, double cut_rnap2, double Delta, int t, FILE* test, int T, FILE* fichier_force_rnap, int periode_enregistrement_force){
    ...
    for (int rnap = 0; rnap < nb_rnap; rnap++) {
        ...
        for (int subunit = 0; subunit<rnap_subunits ; subunit++){
            ...
            for (int subunit2 = 0; subunit2<rnap_subunits ; subunit2++) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    creation_grosse_bille(R,sv->nb_rnap, sv->positions_bille_rnap, sv->R_rnap, cfg->N, cfg->debut_segment, cfg->fin_segment, cfg->nb_rnap_initial);
    sv->R_rnap_new = sv->R_rnap;
    ...
    calcul(sv, cfg, f, neighbor_lists, neighbor_lists_rnap, nbr_simu, endtoend, nb_rnap_2, nb_rnap_initial_2, rnap_subunits_2);
...
void creation_grosse_bille(double** R, int nb_rnap, int* positions_bille_rnap, double*** R_rnap, int N, int debut_segment, int fin_segment, int nb_rnap_initial){
    if (nb_rnap_initial > 0){
        ...
        double x = R[promoteur][0] + (genrand_real2() * 2 - 1);
        double y = R[promoteur][1] + (genrand_real2() * 2 - 1);
        double z = R[promoteur][2] + (genrand_real2() * 2 - 1);
        double R_grosse_bille[3] = {x,y,z}; 

        for (int i = 0; i < N; i++) {  // Parcourir les billes avant le promoteur
            double distance_bille = distance(R_grosse_bille, R[i]);
            if (distance_bille < threshold) {
                ecarter_deux_particules(R[i], R_grosse_bille, threshold);
            }
        }


        R_rnap[rnap][0][0] = R_grosse_bille[0];
        R_rnap[rnap][0][1] = R_grosse_bille[1];
        R_rnap[rnap][0][2] = R_grosse_bille[2];
